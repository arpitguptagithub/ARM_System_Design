 ...
 *
 *
 * 64 characters row wise
 * 37 characters column wise
 *
 * IO_DISPLAY_START = 0
 * display_start = 0
 ...
 *
 *
 *  presently we have support for 128 ascii codes*/

// #include "../core/memory.cpp"
// #include "OS_memory.h"
// #include "Keyboard_ascii_input.cpp"
#include "./../Libararies/font_new.h"
#include<iostream>
#include <fstream>//
#include <bitset>//
#include <vector>//
...
int print_line(char msg[], int len, int line=LINE, int col=COLUMN){
    int e[len];
    for(int i=0;i<len;i++){
        e[i]=getAsciiValue(msg[i]);
    }
    write_string_at_col(e,len,line,col);
}

int main(){
    char msg[]="Hello world @123 \n  I am AlIv$";
    print_line(msg,30,8,3);
    write_char(8);
    int a[SIZE*32]={0};
    std::ofstream outFile("display2.txt");
    std::ofstream out2("display.txt");
    for(int i=0;i<SIZE;i++){
       std::bitset<32> bits(display[i]);  // Convert integer to bitset
       if(i%16==0){
        // std::cout<<"\n";
        out2 << '\n'; // Write to the file
        out2 << bits; // Write to the file
        ...
        outFile << bits; // Write to the file
        outFile << '\n'; // Write to the file
        // for (int j = 0; j < 32; j++) {
        //     a[i *32 + j] = bits[j];  // Store each bit in array 'a'
        // }
    }
    // int memory[240][320]={0};
    // for(int i=0;i<240;i++){
    //     for(int j=0;j<320;j++){
    //         memory[i][j]=a[i+j];
    //     }
