...
This branch contains 5 essential components:

1. **`compiler.y`**: A YACC file that defines a custom programming language's grammar, syntax, and semantic rules. It generates a Three-Address Code (TAC) as the intermediate representation.
2. **`compiler.l`**: A Lex file that contains lexical rules and does token generation
...
9. **Global Variables usage**:
   ...
   - Handled separately with tracking of them in symbol table:
...
#### Purpose
This C++ program translates the TAC generated by `compiler.y` into a sequence of VM instructions suitable for execution on a virtual machine.
...
- **VM Instructions**: Generates VM-compatible code, such as:
  ...
  ```
### Note
- Please refer to the VM branch for full specification of VM code.
---
...
**Makefile Usage**
```bash
make FNAME=<input_parser_filename> INPUTFILE=<input_filename_with_no_file_format> run_vm
```
**Manual Compilation**
1. **Compile `prob1.y`**:
   ```bash
   lex compiler.l
   yacc -v -d -t compiler.y
   g++ -w y.tab.c lex.yy.c -ll -ly -o parser
   ```
2. **Parse Source Code**:
   ```bash
   ./parser < <file_name>.ppc > tac.txt
   ```
3. **Process TAC**:
   ```bash
   python3 global_header_addition.py
   ```
4. **Convert Processed TAC to VM Instructions**:
   - Compile the converter:
     ```bash
     g++ TAC-VM.cpp -o tac_to_vm
     ```
   - Run the converter:
     ```bash
     ./tac_to_vm tac.txt <file_name>.vm
     ```
