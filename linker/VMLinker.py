...
class VMCodeLinker:
  def __init__(self,files):
    ...
    self.input_vmcodes = [open(file,"r").read() for file in (files if isinstance(files, list) else [files])]
    self.target_name = "".join(re.sub(r'[^a-zA-Z0-9_]', '', vmcode.split("/")[-1].split("\\")[-1].split(".")[-2])+"_" for vmcode in files)[:-1]+".vm"
    self.close = lambda : open(self.target_name,"w").write(self.vm_code)

  def get_glob_init_body(self,vmcode,next_label):
    try:
      body = re.search(r"label\s+glob_init\s*([\s\S]*)\s*label\s+end_init\s*",vmcode).group(1)
  ...
  def generate_glob_init_code(self):
    ...
    self.glob_init_code = self.glob_init_code.replace("label end_init","")
    ...
    main_call_code=[
      ...
      f"label end",
      "goto end",
      f"function write_mem 2",
      f"push argument 1",
      f"wmem argument 0",
      f"push constant 0",
      f" return ",
      f"function read_mem 1",
      f"rmem argument 0",
      f" return ",
    ]
    main_call_code_str="\n".join(main_call_code)
  ...
  def get_codes(self,vmcode):
    ...
    return re.sub(r"label\s+glob_init\s*([\s\S]*)\s*label\s+end_init\s*", "", a)
  ...
  def link(self):
    ...
    # replace all occurences of sys_exec("...") with the  call ... 0 : request from os team
    
    self.vm_code = re.sub(r"sys_exec\s*\(\s*\"(.*)\"\s*\)", lambda m: f"call {m.group(1)} 0", self.vm_code)
    
def main():
  import sys
  if len(sys.argv) == 1:
    print(f"Usage: python {sys.argv[0]} <vmcode1> <vmcode2> ...")
    return
  vm_linker = VMCodeLinker(sys.argv[1:])
  vm_linker.link()
  vm_linker.close()
